...
// Zustand Store
const useTabsStore = create<OpenTabs & {
  ...
  // Set Current Tab
  setCurrentTab: (current) => set((state) => {
  ...
  moveTab: (from, to) => set((state) => {
    ...
    if (initialIndex !== -1 && finalIndex !== -1) {
      ...
      updatedOpen.splice(finalIndex + (initialIndex < finalIndex ? 1 : 0), 0, movedTab);
    }
